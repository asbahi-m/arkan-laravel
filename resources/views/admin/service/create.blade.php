...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">{{ __('admin.service_add_new') }}</h4>
        </div>
        <div class="card-body">
                ...
                <form method="POST" action="{{ route('service.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        @forelse ($locales as $locale)
                            ...
                            @enderror
                        @empty
                            <input type="text" id="service-name" class="form-control" name="name" value="{{ old('name') }}" required>
                            @error('name')
                                <div class="invalid-feedback animated fadeInUp" style="display: block;">{{ $message }}</div>
                            @enderror
                        @endforelse
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control default-select" name="type_id">
                            ...
                            @foreach ($types as $type)
                                <option {{ old('type_id') == $type->id || $type->first()->id == $type->id ? 'selected' : '' }}
