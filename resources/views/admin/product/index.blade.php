...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('product.create') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i></a>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover table-responsive-md">
                    ...
                    <tbody>
                        @foreach ($products as $key => $product)
                            <tr>
                                <td><small>{{ $products->firstItem() + $key }}</small></td>
                                <td><small><strong>{{ $product->name }}</strong></small></td>
                                <td><small>{{ $product->type ? Str::upper($product->type_name) : '' }}</small></td>
                                {{-- <td style="width: 33.33%">
                                    <small>{{ Str::limit(strip_tags($product->description), '80', '...') }}</small>
                                </td> --}}
                                <td>
                                    <img src="{{ asset(Storage::url($product->image)) }}" class="rounded-lg" width="80" alt=""/>
                                </td>
                                <td><small>{{ Carbon::create($product->created_at)->locale(app()->getLocale())->isoFormat('ll') }}</small></td>
                                <td>
                                    <small class="d-flex align-items-center">
                                        <i class="fa fa-circle text-{{ $product->is_published == 'published' ? 'success' : 'dark' }} mr-1"></i>
                                        <span>{{ __('admin.' . $product->is_published) }}</span>
                                    </small>
                                </td>
                                <td><small>{{ $product->views_count }}</small></td>
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('product.edit', $product) }}" class="btn btn-info shadow btn-xs sharp mr-1">
                                            <i class="fa fa-pencil"></i></a>
                                        <button class="btn btn-danger shadow btn-xs sharp mr-1" onclick='confirmDelete("{{ $product->id }}")'>
            ...
            <div class="py-2">{{ trans_choice('admin.pagination_info', $products->total(), ['items' => $products->count(), 'total' => $products->total()]) }}</div>
