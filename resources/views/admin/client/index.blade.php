...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('client.create') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i></a>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover table-responsive-md">
                    ...
                    <tbody>
                        @foreach ($clients as $key => $client)
                            <tr>
                                <td><small>{{ $clients->firstItem() + $key }}</small></td>
                                <td><small><strong>{{ $client->name }}</strong></small></td>
                                <td>
                                    <img src="{{ asset(Storage::url($client->image)) }}" class="img-thumbnail rounded-lg" width="120" alt=""/>
                                </td>
                                <td><small>{{ Carbon::create($client->created_at)->locale(app()->getLocale())->isoFormat('ll') }}</small></td>
                                <td>
                                    <small class="d-flex align-items-center">
                                        <i class="fa fa-circle text-{{ $client->is_published == 'published' ? 'success' : 'dark' }} mr-1"></i>
                                        <span>{{ __('admin.' . $client->is_published) }}</span>
                                    </small>
                                </td>
                                <td><small>{{ $client->views_count }}</small></td>
                                <td>
                                    <div class="d-flex">
            ...
            <div class="py-2">{{ trans_choice('admin.pagination_info', $clients->total(), ['items' => $clients->count(), 'total' => $clients->total()]) }}</div>
